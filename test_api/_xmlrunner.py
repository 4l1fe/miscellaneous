...
class _XMLTestResult(_TextTestResult):
    ...
    def addSkip(self, test, reason):
        """Вызывается, когда тест пропускается
        """
        self._prepare_callback(_TestInfo(self, test, _TestInfo.SKIP), \
            self.skipped, 'SKIPED', 'S')
    ...
    def _report_testcase(suite_name, test_result, xml_testsuite, xml_document):
        ...
        if (test_result.outcome == _TestInfo.FAILURE) or (test_result.outcome == _TestInfo.ERROR):
        ...
        if (test_result.outcome == _TestInfo.SKIP):
            elem_name = 'skipped'
            skip = xml_document.createElement(elem_name)
            testcase.appendChild(skip)

            '''
            #failure.setAttribute('type', test_result.err[0].__name__)
            skip.setAttribute('message', "Просто-так вот пропустили тест")
            error_info = str(test_result.get_error_info())
            failureText = xml_document.createCDATASection(error_info)
            failure.appendChild(failureText)
            '''    
    ...
    def generate_reports(self, test_runner):
        ...
        for suite, tests in all_results.items():
            ...
            # Build the XML file
            testsuite = _XMLTestResult._report_testsuite(suite, test_runner.outsuffix, tests, doc)
            for test in tests:
            ...
            if type(test_runner.output) is str:
                report_file = codecs.open('%s%sTEST-%s-%s.xml' % \
                    (test_runner.output, os.sep, suite, \
                     test_runner.outsuffix), 'w', "utf-8")
                try:
